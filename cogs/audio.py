...
class Audio:
    ...
    async def play_local(self, ctx, *, name):
        ...
        if not self.voice_connected(server):
            try:
                ...
                await self.bot.say("You gotta join a voice channel before I can"
                                   " play anything.")
                return
    ...
    async def playlist_queue(self, ctx, url):
        ...
        await self.bot.say("Queued {.title}".format(song))
        ...
        if self._playlist_exists(server, name):
            if not self.voice_connected(server):
                try:
                    ...
                    await self.bot.say("You gotta join a voice channel before"
                                       " I can play anything.")
                    return
                except UnauthorizedConnect:
                    await self.bot.say("I don't have permissions to join your"
    ...
    async def _queue_list(self, ctx):
        ...
        queued_song_list = self._get_queue(server, 20)
        tempqueued_song_list = self._get_queue_tempqueue(server, 20)
    ...
    async def resume(self, ctx):
        ...
        server = ctx.message.server
        if not self.voice_connected(server):
            await self.bot.say("No voices connected in this server.")
            return
    ...
    async def _shuffle(self, ctx):
        ...
        await self.bot.say("Queue has been shuffled.")
